...
impl TunnelStub {
    pub fn new(tsport: Arc<dyn Transport + Send + Sync>, serizer: Arc<Serializer>) -> io::Result<Self> {
    ...
    async fn read_worker(stub: &TunnelStub, tsport: Arc<dyn Transport>, serizer: Arc<Serializer>, sender_send: UnboundedSender<Frame>, closech: UnboundedReceiver<()>) {
    ...
    async fn write_worker(stub: &TunnelStub, sender_recv: UnboundedReceiver<Frame>, close_recv: UnboundedReceiver<()>) {
