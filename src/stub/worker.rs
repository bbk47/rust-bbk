...
use crate::protocol::{Frame, PING_FRAME, PONG_FRAME, INIT_FRAME, EST_FRAME, FIN_FRAME, RST_FRAME};
...
pub struct TunnelStub {
    ...
    streams: Arc<Mutex<HashMap<String, Arc<CopyStream>>>>,
    streamch_send: Sender<Arc<CopyStream>>,
    sender_send: Sender<Frame>,
    pub streamch_recv: Receiver<Arc<CopyStream>>,
                ...
                if let Ok(frame) = serizer2.deserialize(&packet) {
                    ...
                    if frame.r#type == PING_FRAME{
                        ...
                        if let Some(st) = value{
                            streamch_send_cloned.send((*st).clone());
                        }
